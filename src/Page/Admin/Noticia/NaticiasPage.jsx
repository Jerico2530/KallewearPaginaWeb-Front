...
const NoticiasPageAdmin = () => {
  ...
  const nuevoNoticiaForm = useForm(NoticiaInicial, NoticiasValidacion);
  ...
  const {
    ...
    handleEliminar,
    descargarExcel,
  } = useNoticiasAdmin(nuevoNoticiaForm, editNoticiaForm);
  ...
  // Validación extra antes de crear un noticias
  const handleCrearConValidacion = async () => {
    const valido = await nuevoNoticiaForm.validate();
        ...
        <div className="flex flex-col md:flex-row md:items-start gap-4 sm:gap-6 md:gap-8 w-full justify-center">
          ...
          <div className="w-full md:w-1/4 max-w-xs self-start h-fit bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-4 sm:p-6 rounded-2xl shadow-lg space-y-4 sm:space-y-5 border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
              {Object.keys(NoticiaInicial).map((key) =>
                key !== "estado" ? (
                  <div key={key} className="flex flex-col gap-1">
                    <input
                      ...
                      placeholder={key}
                      value={nuevoNoticiaForm.values[key]}
                      onChange={(e) =>
                        nuevoNoticiaForm.handleChange(key, e.target.value)
                      }
                      className={`border rounded-lg w-full px-3 py-2 text-sm sm:text-base focus:ring-2 ${
                        nuevoNoticiaForm.errors[key]
              ...
              {/* Selector de estado */}
              <select
                value={nuevoNoticiaForm.values.estado ? "true" : "false"}
                onChange={(e) =>
                  nuevoNoticiaForm.handleChange(
                    "estado",
                    e.target.value === "true"
                  )
                }
