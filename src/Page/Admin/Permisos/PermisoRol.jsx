...
const UserPermRolPageAdmin = () => {
  ...
  const nuevoPermRolForm = useForm(PermRolInicial, PermRolValidacion);
  const editPermRolForm = useForm(PermRolInicial, PermRolValidacion);
  ...
  const {
    ...
    handleEliminar,
    descargarExcel,
  } = usePermRolAdmin(nuevoPermRolForm, editPermRolForm);

  /** Opciones para selects: Permiso y Rol */
   ...
   */
  const columns = [
    { key: "nombrePermiso", label: "Permiso" },
  ...
  const handleCrearConValidacion = async () => {
    const valido = await nuevoPermRolForm.validate();
    ...
    <PageCrud activeTab="permrol">
      <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-lg rounded-2xl max-w-full mx-auto p-4 sm:p-6 space-y-6 border dark:border-gray-700">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-center border-b pb-4 dark:border-secondary text-gray-800 dark:text-gray-100">
          Administración de permisos Rol
        </h1>
          ...
          <div className="w-full md:w-1/4 max-w-xs self-start h-fit bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-4 sm:p-6 rounded-2xl shadow-lg space-y-4 sm:space-y-5 border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
              ...
              <Select
                ...
                value={
                  permisoOptions.find(
                    (opt) => opt.value === nuevoPermRolForm.values.permisoId
                  ) || null
                }
                onChange={(selected) =>
                  nuevoPermRolForm.handleChange(
                    "permisoId",
              ...
              <Select
                ...
                value={
                  rolOptions.find(
                    (opt) => opt.value === nuevoPermRolForm.values.rolId
                  ) || null
                }
                onChange={(selected) =>
                  nuevoPermRolForm.handleChange(
              ...
              {/* Select estado */}
              <select
                value={nuevoPermRolForm.values.estado ? "true" : "false"}
                onChange={(e) =>
                  nuevoPermRolForm.handleChange(
                    "estado",
                    e.target.value === "true"
            ...
            <div className="overflow-x-auto scrollbar-none">
              {isLoading ? (
                ...
                <DataTable
                  ...
                  columns={columns}
                  editId={editandoId}
                  editData={editPermRolForm.values}
                  onEditChange={editPermRolForm.handleChange}
